...
func insertRoundPlayerStats(ctx context.Context, tx pgx.Tx, rows []aggregate.RoundPlayerStatsRow) error {
	...
	columns := []string{
		...
		"first_kill", "first_death", "trade_kill", "traded_death",
	...
	_, err := tx.CopyFrom(
		...
		pgx.CopyFromSlice(len(rows), func(i int) ([]any, error) {
			...
			return []any{
				...
				r.FirstKill, r.FirstDeath, r.TradeKill, r.TradedDeath,
...
func insertMatchPlayerStats(ctx context.Context, tx pgx.Tx, rows []aggregate.MatchPlayerStatsRow) error {
	...
	columns := []string{
		...
		"trade_kills", "traded_deaths", "multi_kills",
	...
	_, err := tx.CopyFrom(
		...
		pgx.CopyFromSlice(len(rows), func(i int) ([]any, error) {
			...
			return []any{
				...
				r.TradeKills, r.TradedDeaths, r.MultiKills,
...
func insertTeamMatchStats(ctx context.Context, tx pgx.Tx, rows []aggregate.TeamMatchStatsRow) error {
	...
	columns := []string{
		...
		"trade_kills", "traded_death", "multikill",
	...
	_, err := tx.CopyFrom(
		...
		pgx.CopyFromSlice(len(rows), func(i int) ([]any, error) {
			...
			return []any{
				...
				r.TradeKills, r.TradedDeaths, r.Multikill,
